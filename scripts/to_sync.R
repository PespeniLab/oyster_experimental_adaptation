...
dat <- read.table("~/oyster/analysis/filtered_variants.txt", header=TRUE, stringsAsFactors=FALSE)

pops <- c("LL_33", "LL_05", "OP_33", "OP_05", "TB_33", "TB_05")


syncOut <- as.data.frame(matrix(nrow=nrow(dat), ncol=9))
...
write.table(syncOut, file="~/oyster/analysis/variants.sync", col.names=FALSE, quote=FALSE, row.names=FALSE, sep="\t")
